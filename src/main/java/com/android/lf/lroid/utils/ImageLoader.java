...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = new File(SDCardUtils.getSDCardPath(), "Lroid/image/cache");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, 1, 1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    class DiskCacheThread extends Thread {
        ...
        public void run() {
            try {
                String key = MethodUtils.hashKeyForDisk(path);
